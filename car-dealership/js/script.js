...
function  produceCars() {
  ...
  makeCar('bmw', 'german', 'img/car-american-4.jpeg', undefined , 'other model');
...
// document loaded event
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    specialCars.forEach(item=>{
      data +=`
      <!-- single item -->
          <div class="featured-item my-3 d-flex p-2 text-capitalize align-items-baseline flex-wrap" data-img="${item.img}">
...
if (event.target.parentElement.dataset.img) {
    // const img = event.target.parentElement.dataset.img;
      const img = event.target.parentElement.dataset.img;
    document.querySelector('.featured-photo').src = img;
  } 
  else if (event.target.parentElement.parentElement.dataset.img ) {
    const img = event.target.parentElement.parentElement.dataset.img;
    document.querySelector('.featured-photo').src = img;
  }
   else if (event.target.parentElement.parentElement.parentElement.dataset.img ) {
    const img = event.target.parentElement.parentElement.parentElement.dataset.img;
    document.querySelector('.featured-photo').src = img;
  }
  else if (event.target.parentElement.parentElement.parentElement.parentElement.dataset.img ) {
    const img = event.target.parentElement.parentElement.parentElement.parentElement.dataset.img;
    document.querySelector('.featured-photo').src = img;}
  else if (event.target.dataset.img ) {
    ...
    cars.forEach((car) =>{
      ...
      <div class="col-10 mx-auto my-3 col-md-6 col-lg-4 single-car ${car.country}">
          <div class="card car-card">
            <img src="${car.img}" class="card-image-top car-img img-fluid" alt="gc">
            <!-- card body -->
            <div class="card-body">
